...
-(BOOL)isClickedToday{
    ...
    NSDate *lastDate = [defaults objectForKey:@"lastClickDate"];
    
    if(lastDate == nil)
        return NO;
    return [self isSameDay:currentDay otherDay:lastDate];
}
...
-(void)setClickDate{
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [defaults setObject:today forKey:@"lastClickDate"];
...
- (IBAction)facebookShare:(id)sender {
    ...
    if([self isClickedToday]){
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Sorry !!!!" message:@"You have already shared Today Please try tomorrow, Thank You" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
    ...
    //NSString* szCurMessage = [NSString stringWithFormat:@"I am Playing Popstar Bubbles Game  ,Its awesome to play matchup game /n "];
    
    SLComposeViewController *facebook=[SLComposeViewController composeViewControllerForServiceType:SLServiceTypeFacebook];
    
    NSString* szCurMessage = [NSString stringWithFormat:@"This Slot game is becoming trending game on appstore Play now and win coins just like me!"];
    
    //add your text message here
    
    [facebook setInitialText:szCurMessage];
    ...
    [facebook addURL:[NSURL URLWithString:@"https://itunes.apple.com/app/id1002974994"]];
    
    UIWindow *window = (UIWindow *)[[[UIApplication sharedApplication] windows] objectAtIndex:1];
    
    [window.rootViewController presentViewController:facebook animated:YES completion:nil];
    ...
    [facebook setCompletionHandler:^(SLComposeViewControllerResult result) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Post Confirmation:" message:output delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
...
- (IBAction)removeadsBtn:(id)sender {
       if(self.productsArray.count > 0 )
    {
        SKProduct *toBeBuyProduct = [[SKProduct alloc] init];
        toBeBuyProduct = (SKProduct*)[self.productsArray objectAtIndex:0];
        [[GuessIAPHelper sharedInstance] buyProduct:toBeBuyProduct];
    }
    else
    {
        NSLog(@"Error in loading products");
    }
...
- (IBAction)restoreBtn:(id)sender {
    ...
    // [GuessIAPHelper restoreCompletedTransactions];
}

-(void)loadInAppPurchases {
    self.productsArray   = nil;
    [[GuessIAPHelper sharedInstance] requestProductsWithCompletionHandler:^(BOOL success, NSArray *products) {
        if (success) {
            self.productsArray = products;
            NSLog(@"Product array count==%d",self.productsArray.count);
            for (int i = 0; i< [products count]; i++) {
                NSLog(@"Before: index = %d, identifier = %@", i, ((SKProduct *) [self.productsArray objectAtIndex:i]).productIdentifier);
            }
...
//    moveSlides = floor(games.contentOffset.y / 0);
    }else{
